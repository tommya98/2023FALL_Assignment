...
int main(void)
{
  ...
  xTaskCreate(ledTask, "ledTask", STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
...
void EXTI15_10_IRQHandler(void) {
	...
	long *const xHigherPriorityTtaskWoekn = pdFALSE;

	xSemaphoreGiveFromISR(semPtr, xHigherPriorityTtaskWoekn);
}

void ledTask( void * NotUsed) {
	LedStates_t states;
	states.redLEDState = 1;
	states.blueLEDState = 0;
	while(1) {
		if(xSemaphoreTake(semPtr, portMAX_DELAY) == pdPASS) {
			states.redLEDState = (states.redLEDState + 1) % 2;
			states.blueLEDState = (states.blueLEDState + 1) % 2;
		}
		if(states.redLEDState == 1) RedLed.On();
		else RedLed.Off();
		if(states.blueLEDState == 1) BlueLed.On();
		else BlueLed.Off();
	}
}
